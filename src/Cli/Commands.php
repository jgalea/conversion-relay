...
final class Commands {
	...
	public function status(): void {
		...
		if ( empty( $counts ) ) {
			\WP_CLI::log( __( 'No events recorded yet.', 'conversion-relay' ) );
			return;
		}
	...
	public function destinations(): void {
		...
		if ( empty( $rows ) ) {
			\WP_CLI::log( __( 'No destinations registered.', 'conversion-relay' ) );
			return;
		}
	...
	public function test_event( array $args, array $assoc ): void {
		...
		if ( ! EventType::is_valid( $type ) ) {
			\WP_CLI::error(
				sprintf(
					...
					__( 'Invalid event type: %s', 'conversion-relay' ),
					$type
				)
			);
		}
		...
		\WP_CLI::success(
			sprintf(
				...
				__( 'Dispatched a %s event. Check `wp conversion-hub status`.', 'conversion-relay' ),
				$type
			)
		);
	}
	...
	public function retry(): void {
		...
		\WP_CLI::success(
			sprintf(
				...
				__( 'Re-queued %d failed deliveries.', 'conversion-relay' ),
				$n
			)
		);
	}
